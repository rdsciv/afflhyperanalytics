...
function playersView(){
  ...
  const draw = ()=>{
    ...
    if(playerScope==='affl'){
      ...
      const key = {affl:r=>r.affl, wks:r=>r.wks, starts:r=>r.sts, usd:r=>r.usd, nfp:r=>r.nfp, fpoe:r=>r.fpoe}[playerSort]||(r=>r.affl);
      ...
      $('#plist').innerHTML = `<div class="tblwrap"><table class="tbl"><thead><tr>
        ...
        ${list.map((r,i)=>{
          ...
          <td data-v="${r.fpoe}" class="${cls(r.fpoe)}">${signed(r.fpoe,f1)}</td>
      ...
      const key = {nfp:r=>r.c.fp, fppg:r=>r.c.g?r.c.fp/r.c.g:0, xfp:r=>r.c.xfp2, fpoe:r=>r.c.fpoe2, g:r=>r.c.g}[playerSort]||(r=>r.c.fp);
      rows.sort((a,b)=>key(b)-key(a));
      const list = rows.slice(0,300);
      $('#plist').innerHTML = `<div class="tblwrap"><table class="tbl"><thead><tr>
        ...
        ${list.map((r,i)=>{
          ...
          <td data-v="${r.c.xfp2}">${f1(r.c.xfp2)}</td>
          <td data-v="${r.c.fpoe2}" class="${cls(r.c.fpoe2)}">${signed(r.c.fpoe2,f1)}</td>
...
function seasonTableHtml(pi, pos, p){
  ...
  const head = `<tr><th class="l">Year</th><th class="l">Team</th><th>G</th>
    ...
    <th>FP</th><th>FP/G</th><th title="Expected FP, xfp_v2 (ffopportunity canon, AFFL-rescored)">xFP</th>
    <th title="FP over expected, v2">FPOE</th>${!isK?'<th title="opponent-adjusted FPOE (adjfac_v1)">aFPOE</th>':''}
  ...
  const body = rows.map(r=>{
    ...
    <td><b>${f1(r[SCOL.fp])}</b></td><td>${f1(g?(r[SCOL.fp]||0)/g:null)}</td><td>${f1(r[SCOL.xfp2])}</td>
    <td class="${cls(r[SCOL.fpoe2])}">${signed(r[SCOL.fpoe2],f1)}</td>
    ${!isK?`<td class="${cls(r[SCOL.afpoe2])}">${signed(r[SCOL.afpoe2],f1)}</td>`:''}
...
function logTableHtml(logs, s, pos){
  ...
  const head = `<tr><th>Wk</th><th class="l">Tm</th><th class="l">Opp</th><th class="l" title="AFFL custody: franchise + started/bench">AFFL</th>
    ...
    ${!isK?'<th title="fumbles lost">FmL</th>':''}
    <th>FP</th><th>xFP</th><th>FPOE</th></tr>`;
  const body = rows.map(r=>{
    if(r.dnp) return `<tr class="dnp"><td>${r.w}</td><td class="l" colspan="2"><span class="dim">no NFL game — bye / inactive</span></td>
      <td class="l">${affl(r)}</td><td class="l dim small" colspan="${(isQB?9:isRB?7:isRec?6:isK?4:0)+(isK?0:1)+3}">${r.cust&&r.cust.affl!=null?'AFFL applied: '+f1(r.cust.affl)+' pts':''}</td></tr>`;
    ...
    <td><b>${f1(r.fp)}</b></td><td class="dim">${f1(r.xfp)}</td><td class="${cls(r.fpoe)}">${signed(r.fpoe,f1)}</td></tr>`;
  }).join('');
...
function drawSeasonArc(pi, eid){
  ...
  const xfpg = rows.map(r=>r[SCOL.g]&&r[SCOL.xfp2]?+((r[SCOL.xfp2]||0)/r[SCOL.g]).toFixed(2):null);
  ...
  mkChart('plarc', {
    ...
    series: [
      ...
      {name:'xFP / game', type:'line', data:xfpg, smooth:.3, symbol:'none',
...
function drawWeeklyBeat(pi){
  ...
  mkChart('plbeat', {
    tooltip: ECTT({formatter: ps=>{
      ...
      return `<b>${r.s} Week ${r.w}</b> · ${esc(r.tm||'')}${r.opp?' v '+esc(r.opp):''}<br>FP <b>${f1(r.fp)}</b> · xFP ${f1(r.xfp)} · FPOE ${signed(r.fpoe,f1)}<br><span style="color:#8fa2bd">${cu}</span>`; }}),
    ...
    series: [
      ...
      {name:'xFP', type:'line', data:logs.map(r=>r.xfp), symbol:'none', lineStyle:{width:1.4, color:'#c8ff00', opacity:.85}, z:3},
    ],
  });
}
...
function playerView(eid){
  ...
  app.innerHTML = `<div class="wrap">
    <div class="phero" style="background:linear-gradient(120deg, ${acc}26, var(--card) 58%);border-color:${acc}55">
      ...
      <div style="min-width:0;flex:1">
        ...
        <div class="statline">
          ...
          <div class="stat"><div class="v ${cls(cn.fpoe2)}">${signed(cn.fpoe2,f1)}</div><div class="l">Career FPOE</div></div>`:''}
        </div>
      </div></div>
    ...
    <div class="grid g2" style="margin-top:10px"><div class="card" id="pctwrap"></div>
      ...
      <p class="dim small" style="margin-top:6px">Blue = FP/game · dashed lime = expected FP/game (xfp_v2) · gold bars = AFFL auction price paid that year.</p></div></div>
    ...
    ${pi!=null?`<div class="presets" style="margin-top:18px">
      <a class="chip" href="#/explore?q=${encState(Object.assign(defState(),{pq:p.name,gr:'weeks',sc:'rostered',ms:['affl','fp','tgt','car'],sort:'affl',lim:100}))}">▦ Every custody week in Explore</a>
      <a class="chip" href="#/compare?p=${pi}">⇄ Compare with…</a>
    </div>`:''}
  </div>`;
  ...
  if(hasNfl){
    ...
    const drawLogs = ()=>{
      ...
      $('#glwrap').innerHTML = logTableHtml(logs, glSeason, pos)
        + `<p class="dim small" style="margin-top:8px">FP is nflverse std non-PPR (fumbles included); xFP/FPOE are xfp_v2 (no fumble/kicking term). ● on the opponent marks defense-vs-${esc(pos)} generosity that season. Pre-2018 AFFL columns show starters only — bench custody is unobservable.</p>`;
