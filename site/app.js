...
function chrome(route){
  const active = ({f:'franchises', p:'players', pn:'players', s:'seasons'})[route] || route;
  const nav=$('#nav');
  nav.innerHTML = NAV.map(([r,l]) =>
    `<a href="#/${r}" class="${r==='explore'?'explore-cta':''} ${active===r?'on':''}">${l}</a>`).join('');
  requestAnimationFrame(()=>nav.querySelector('a.on')?.scrollIntoView({block:'nearest',inline:'center'}));
}
function render(){
  ...
  try{
    ...
    else if(r==='pn') nflPlayerView(+seg[1]);
...
function home(){
  ...
  <div class="wrap">
    ...
    <h2 class="sect">Record book highlights <span class="sub">regular season</span></h2>
    <div class="grid g3">
      ...
      <div class="card"><div class="kicker">Biggest blowout</div>
        ...
        <div class="dim small">${bl.season} · Wk ${bl.mp} · <a href="#/records">full record book →</a></div></div>
    </div>
...
function drawCmpRadar(sel, rowsBy){
  ...
  const careerOf = rs=>{
    const pi=rs[0]?.[SCOL.p]; if(pi==null||EPOS[pi]==='K') return null;
...
function franchisesView(){
  ...
  const rivalCards = [
    ...
    marg? (function(){
      const aWins = marg.apf>marg.bpf;
      const wn = aWins?marg.an:marg.bn, ln = aWins?marg.bn:marg.an;
      const wavg = Math.max(marg.apf,marg.bpf)/marg.g, lavg = Math.min(marg.apf,marg.bpf)/marg.g;
      const wrec = aWins? `${marg.aw}–${marg.bw}` : `${marg.bw}–${marg.aw}`;
      return rc('💪','Widest average margin', `${esc(wn)} over ${esc(ln)}`,
        `by ${f1(wavg-lavg)}/game · ${f1(wavg)}–${f1(lavg)} avg score · ${wrec}${marg.t?'–'+marg.t:''} in ${marg.g} meetings`);
    })():'',
...
function franchiseView(fid){
  ...
  app.innerHTML = `<div class="wrap">
    ...
    <div class="presets">
      <a class="chip" href="#/explore?q=${encState(Object.assign(defState(),{sc:'started',gr:'playerSeason',f:[fid],ms:['affl','fp','xfp2','fpoe2'],sort:'affl'}))}">▦ Best player-seasons started</a>
...
/* ================= PLAYERS ================= */
/* Player-profile layer: PlayerProfiler-style percentile profile + grouped
   AFFL résumé with league-internal ranks, Sleeper-clean game logs (custody
   weeks from EXPLORE + non-custody weeks from the lazy GAMELOGS mart),
   FantasyPros-style position-aware season tables. */
const TEAMC = {ARI:'#97233F',ATL:'#A71930',BAL:'#241773',BUF:'#00338D',CAR:'#0085CA',CHI:'#C83803',
 CIN:'#FB4F14',CLE:'#FF3C00',DAL:'#0053A0',DEN:'#FB4F14',DET:'#0076B6',GB:'#37543A',HOU:'#03202F',
 IND:'#0057B8',JAX:'#006778',JAC:'#006778',KC:'#E31837',LA:'#003594',LAC:'#0080C6',LV:'#A5ACAF',
 MIA:'#008E97',MIN:'#4F2683',NE:'#0A2342',NO:'#D3BC8D',NYG:'#0B2265',NYJ:'#125740',PHI:'#004C54',
 PIT:'#FFB612',SEA:'#69BE28',SF:'#AA0000',TB:'#D50A0A',TEN:'#4B92DB',WAS:'#873B26',
 OAK:'#A5ACAF',SD:'#0080C6',STL:'#003594'};
const teamAccent = tm => TEAMC[tm] || '#31405e';
const plRoute = pi => { const e=(E.players[pi]||[])[0]; return e!=null? '#/p/'+e : '#/pn/'+pi; };
function headshotUrl(p){
  if(p.img) return p.img;
  if(p.eid && p.eid>0 && !p.dst) return 'https://a.espncdn.com/i/headshots/nfl/players/full/'+p.eid+'.png';
  return null;
}

/* ---- lazy gamelogs (inline on the artifact build; fetched on Pages) ---- */
let _glp = null;
function ensureGL(cb){
  if(window.GAMELOGS) return cb(true);
  const urls = window.GAMELOGS_URLS;
  if(!urls || !urls.length) return cb(false);
  if(!_glp) _glp = Promise.all(urls.map(u=>fetch(u).then(r=>{ if(!r.ok) throw new Error(u); return r.json(); })))
    .then(parts=>{ const g=parts[0];
      for(let i=1;i<parts.length;i++){ g.rows=g.rows.concat(parts[i].rows||[]); g.custRows=(g.custRows||[]).concat(parts[i].custRows||[]); }
      window.GAMELOGS=g; return true; })
    .catch(()=>false);
  _glp.then(cb);
}

/* ---- merged weekly logs: custody rows (explore) + non-custody rows ---- */
function playerLogs(pi){
  const C = COL, out = [];
  const gl = window.GAMELOGS;
  const ext = {};
  let GC=null, GCC=null;
  if(gl){
    GC={}; gl.cols.forEach((c,i)=>GC[c]=i);
    GCC={}; gl.custCols.forEach((c,i)=>GCC[c]=i);
    gl.custRows.forEach(r=>{ if(r[GCC.p]===pi) ext[r[GCC.s]*100+r[GCC.w]]=r; });
  }
  E.rows.forEach(r=>{
    if(r[C.p]!==pi) return;
    const x = ext[r[C.s]*100+r[C.w]];
    const played = r[C.fp]!=null;
    out.push({s:r[C.s], w:r[C.w], tm:r[C.tm], opp:r[C.opp], fp:r[C.fp], xfp:r[C.xfp2], fpoe:r[C.fpoe2],
      cmp:x?x[GCC.cmp]:(played?0:null), att:x?x[GCC.att]:(played?0:null),
      pyd:r[C.pyd], ptd:r[C.ptd], int:r[C.int], sk:r[C.sk],
      car:r[C.car], ryd:r[C.ryd], rtd:r[C.rtd], tgt:r[C.tgt], rec:r[C.rec], recyd:r[C.recyd], rectd:r[C.rectd],
      fuml:x?x[GCC.fuml]:(played?0:null),
      fgm:x?x[GCC.fgm]:(played?0:null), fga:x?x[GCC.fga]:(played?0:null),
      xpm:x?x[GCC.xpm]:(played?0:null), xpa:x?x[GCC.xpa]:(played?0:null),
      cust:{fid:E.franchises[r[C.f]][0], st:r[C.st], ev:r[C.ev], affl:r[C.affl], slot:r[C.slot]}, dnp:!played});
  });
  if(gl) gl.rows.forEach(r=>{
    if(r[GC.p]!==pi) return;
    out.push({s:r[GC.s], w:r[GC.w], tm:r[GC.tm], opp:r[GC.opp], fp:r[GC.fp], xfp:r[GC.xfp2], fpoe:r[GC.fpoe2],
      cmp:r[GC.cmp], att:r[GC.att], pyd:r[GC.pyd], ptd:r[GC.ptd], int:r[GC.int], sk:r[GC.sk],
      car:r[GC.car], ryd:r[GC.ryd], rtd:r[GC.rtd], tgt:r[GC.tgt], rec:r[GC.rec], recyd:r[GC.recyd], rectd:r[GC.rectd],
      fuml:r[GC.fuml], fgm:r[GC.fgm], fga:r[GC.fga], xpm:r[GC.xpm], xpa:r[GC.xpa], cust:null, dnp:false});
  });
  out.sort((a,b)=>a.s-b.s || a.w-b.w);
  return out;
}

/* ---- defense-vs-position difficulty for the opponent chip ---- */
let _defIdx = null;
function defIdx(){
  if(_defIdx) return _defIdx;
  const m = {}, ranks = {};
  const DB = {}; (E.defBoardCols||[]).forEach((c,i)=>DB[c]=i);
  const byGrp = {};
  (E.defBoards||[]).forEach(r=>{
    m[r[DB.s]+'|'+r[DB.def]+'|'+r[DB.pos]] = r[DB.idx];
    (byGrp[r[DB.s]+'|'+r[DB.pos]] = byGrp[r[DB.s]+'|'+r[DB.pos]]||[]).push(r);
  });
  Object.entries(byGrp).forEach(([k, rows])=>{
    rows.sort((a,b)=>(b[DB.idx]||0)-(a[DB.idx]||0));
    rows.forEach((r,i)=>ranks[r[DB.s]+'|'+r[DB.def]+'|'+r[DB.pos]] = i+1);
  });
  _defIdx = {idx:m, rank:ranks};
  return _defIdx;
}
function oppCell(s, opp, pos){
  if(!opp) return '·';
  if(!pos || pos==='K' || pos==='D/ST') return esc(opp);
  const d = defIdx();
  const k = s+'|'+opp+'|'+pos, v = d.idx[k];
  if(v==null) return esc(opp);
  const col = v>=1.12? 'var(--green)' : v<=0.88? 'var(--red)' : 'var(--ink3)';
  return `${esc(opp)} <span style="color:${col};cursor:default" title="${esc(opp)} allowed ${v}× the league-average FP to ${esc(pos)}s in ${s} (rank ${d.rank[k]} of 32, 1 = most generous)">●</span>`;
}

/* ---- percentile machinery over seasonRows (position peers) ---- */
const AGG_KEYS = ['g','fp','xfp2','fpoe2','afpoe2','pyd','ptd','pint','cmp','att','sk','car','ryd','rtd',
 'gl','rzc','tgt','rec','recyd','rectd','air','yac','rztgt','eztgt','recepa','repa','pepa','db',
 'fuml','fgm','fga','xpm','xpa'];
function aggInit(){ const a={}; AGG_KEYS.forEach(k=>a[k]=0); a.adotW=0; a.cpoeW=0; a.tshareW=0; a.woprW=0; return a; }
function aggAdd(a, r){
  AGG_KEYS.forEach(k=>{ const v=r[SCOL[k]]; if(v!=null) a[k]+=v; });
  const g=r[SCOL.g]||0, tgt=r[SCOL.tgt]||0, db=r[SCOL.db]||0;
  if(r[SCOL.adot]!=null) a.adotW += r[SCOL.adot]*tgt;
  if(r[SCOL.cpoe]!=null) a.cpoeW += r[SCOL.cpoe]*db;
  if(r[SCOL.tshare]!=null) a.tshareW += r[SCOL.tshare]*g;
  if(r[SCOL.wopr]!=null) a.woprW += r[SCOL.wopr]*g;
  return a;
}
function pQual(pos, a, career){
  const m = career?2:1;
  if((a.g||0) < (career?16:6)) return false;
  if(pos==='QB') return a.att >= 100*m;
  if(pos==='RB') return (a.car+a.tgt) >= 50*m;
  if(pos==='WR') return a.tgt >= 30*m;
  if(pos==='TE') return a.tgt >= 20*m;
  if(pos==='K')  return a.fga >= 10*m;
  return true;
}
const _pmCommon = [
 ['fpg','FP / game', a=>a.g?a.fp/a.g:null, f1],
 ['xfpg','Expected FP / game', a=>a.g&&a.xfp2?a.xfp2/a.g:null, f1],
 ['fpoeg','FPOE / game', a=>a.g&&a.xfp2?a.fpoe2/a.g:null, v=>signed(v,f2)],
];
const _pmRec = [
 ['tgtg','Targets / game', a=>a.g?a.tgt/a.g:null, f1],
 ['recydg','Rec yards / game', a=>a.g?a.recyd/a.g:null, f1],
 ['ctch','Catch rate', a=>a.tgt?a.rec/a.tgt:null, pct1],
 ['ypt','Yards / target', a=>a.tgt?a.recyd/a.tgt:null, f2],
 ['adot','aDOT', a=>a.tgt&&a.adotW?a.adotW/a.tgt:null, f1],
 ['tsh','Target share', a=>a.g&&a.tshareW?a.tshareW/a.g:null, pct1],
 ['wopr','WOPR', a=>a.g&&a.woprW?a.woprW/a.g:null, f2],
 ['epat','EPA / target', a=>a.tgt?a.recepa/a.tgt:null, f2],
 ['rztgtg','RZ targets / game', a=>a.g?a.rztgt/a.g:null, f2],
];
const PM = {
 QB: [..._pmCommon,
  ['pydg','Pass yards / game', a=>a.g?a.pyd/a.g:null, f1],
  ['ptdg','Pass TD / game', a=>a.g?a.ptd/a.g:null, f2],
  ['cmppct','Completion %', a=>a.att?a.cmp/a.att:null, pct1],
  ['ypa','Yards / attempt', a=>a.att?a.pyd/a.att:null, f2],
  ['intg','INT / game', a=>a.g?a.pint/a.g:null, f2, 1],
  ['epadb','EPA / dropback', a=>a.db?a.pepa/a.db:null, f2],
  ['cpoe','CPOE', a=>a.db&&a.cpoeW?a.cpoeW/a.db:null, v=>v==null?'·':signed(v,x=>x.toFixed(1))+'%'],
  ['rydg','Rush yards / game', a=>a.g?a.ryd/a.g:null, f1]],
 RB: [..._pmCommon,
  ['carg','Carries / game', a=>a.g?a.car/a.g:null, f1],
  ['rydg','Rush yards / game', a=>a.g?a.ryd/a.g:null, f1],
  ['ypc','Yards / carry', a=>a.car?a.ryd/a.car:null, f2],
  ['glg','Goal-line carries / game', a=>a.g?a.gl/a.g:null, f2],
  ['epac','EPA / carry', a=>a.car?a.repa/a.car:null, f2],
  ['tgtg','Targets / game', a=>a.g?a.tgt/a.g:null, f1],
  ['recydg','Rec yards / game', a=>a.g?a.recyd/a.g:null, f1],
  ['tsh','Target share', a=>a.g&&a.tshareW?a.tshareW/a.g:null, pct1]],
 WR: [..._pmCommon, ..._pmRec],
 ...
 K: [
  ['fpg','FP / game', a=>a.g?a.fp/a.g:null, f1],
  ['fgmg','FG made / game', a=>a.g?a.fgm/a.g:null, f2],
  ['fgag','FG attempts / game', a=>a.g?a.fga/a.g:null, f2],
  ['fgpct','FG %', a=>a.fga?a.fgm/a.fga:null, pct1],
  ['xpg','XP made / game', a=>a.g?a.xpm/a.g:null, f2]],
};
let _pools = null;
function pctPools(){
  if(_pools) return _pools;
  const seasonAggs = {}, careerAggs = {}, rowsByPi = new Map();
  E.seasonRows.forEach(r=>{
    const pi=r[SCOL.p], pos=EPOS[pi]; if(!pos||!PM[pos]) return;
    if(!rowsByPi.has(pi)) rowsByPi.set(pi, []);
    rowsByPi.get(pi).push(r);
    const sk = pos+'|'+r[SCOL.s];
    (seasonAggs[sk]=seasonAggs[sk]||[]).push(aggAdd(aggInit(), r));
  });
  rowsByPi.forEach((rs, pi)=>{
    const pos=EPOS[pi];
    const a=aggInit(); rs.forEach(r=>aggAdd(a,r));
    (careerAggs[pos]=careerAggs[pos]||[]).push(a);
  });
  _pools = {seasonAggs, careerAggs, rowsByPi, sorted:{}};
  return _pools;
}
function pctOf(pos, scope, mk, fn, v, inv, career){
  if(v==null) return null;
  const P = pctPools();
  const ck = pos+'|'+scope+'|'+mk;
  let vals = P.sorted[ck];
  if(!vals){
    const aggs = (scope==='career'? P.careerAggs[pos] : P.seasonAggs[pos+'|'+scope]) || [];
    vals = aggs.filter(a=>pQual(pos,a,career)).map(fn).filter(x=>x!=null&&!isNaN(x)).sort((a,b)=>a-b);
    P.sorted[ck] = vals;
  }
  if(!vals.length) return null;
  let lo=0, hi=vals.length;
  while(lo<hi){ const m=(lo+hi)>>1; if(vals[m]<=v) lo=m+1; else hi=m; }
  const p = Math.round(100*lo/vals.length);
  if(!inv) return {p, n:vals.length};
  let lo2=0, hi2=vals.length;
  while(lo2<hi2){ const m=(lo2+hi2)>>1; if(vals[m]<v) lo2=m+1; else hi2=m; }
  return {p:Math.round(100*(vals.length-lo2)/vals.length), n:vals.length};
}
const pctColor = p => `hsl(${Math.round(p*1.2)},68%,50%)`;
function pctBarsHtml(pos, agg, scope, career){
  const rows = (PM[pos]||[]).map(([mk,l,fn,fmt,inv])=>{
    const v = fn(agg);
    const pr = pctOf(pos, scope, mk, fn, v, inv, career);
    if(v==null||pr==null) return '';
    return `<div class="pct-row" title="${esc(l)}: ${pr.p}th percentile among ${pr.n} qualified ${esc(pos)} ${career?'careers':scope+' seasons'}">
      <span class="pl">${esc(l)}</span>
      <span class="pbw"><span class="pbf" style="width:${Math.max(2,pr.p)}%;background:${pctColor(pr.p)}"></span></span>
      <span class="pv">${fmt(v)}</span><span class="pp" style="color:${pctColor(pr.p)}">${pr.p}</span></div>`;
  }).filter(Boolean).join('');
  return rows || '<p class="dim small">Not enough volume for a percentile profile in this scope.</p>';
}

/* ---- league-rank memos for the AFFL résumé ---- */
let _lr = null;
function leagueRanks(){
  if(_lr) return _lr;
  const C = COL;
  const bestWk = {}, benchLeft = {}, afflWk = {};
  const regW = {}; Object.keys(S.seasons).forEach(y=>regW[y]=S.seasons[y].regWeeks||14);
  E.rows.forEach(r=>{
    const pi=r[C.p], v=r[C.affl];
    if(v==null) return;
    afflWk[pi]=(afflWk[pi]||0)+1;
    if(r[C.w] > (regW[r[C.s]]||14)) return;   // record-book scope: regular season only
    if(r[C.st]){ const b=bestWk[pi]; if(!b||v>b.v) bestWk[pi]={v, s:r[C.s], w:r[C.w], fid:E.franchises[r[C.f]][0]}; }
    else if(r[C.s]>=2018) benchLeft[pi]=(benchLeft[pi]||0)+v;
  });
  const dr = {};
  S.drafts.forEach(d=>{
  ...
  const tr = {};
  S.trades.forEach(t=>Object.values(t.sides).forEach(eids=>eids.forEach(e=>{ tr[e]=(tr[e]||0)+1; })));
  const rankMap = (pairs, desc=true)=>{
    const sorted=[...pairs].sort((a,b)=>desc?b[1]-a[1]:a[1]-b[1]);
    const m={}; sorted.forEach(([k],i)=>m[k]=i+1); return {m, n:sorted.length};
  };
  _lr = {
    bestWk, benchLeft, dr, tr,
    afflPts: rankMap(S.players.filter(p=>p.afflPts>0).map(p=>[p.eid,p.afflPts])),
    weeks: rankMap(S.players.filter(p=>p.stints.length).map(p=>[p.eid,p.stints.reduce((a,s)=>a+s.weeks,0)])),
    usd: rankMap(Object.entries(dr).filter(([,o])=>o.usd>0).map(([e,o])=>[e,o.usd])),
    dpar: rankMap(Object.entries(dr).filter(([,o])=>o.n>0).map(([e,o])=>[e,o.par])),
    bwk: rankMap(Object.entries(bestWk).map(([pi,b])=>[pi,b.v])),
  };
  return _lr;
}
const rkChip = (rk, n)=> rk? `<span class="rk">#${rk} <span class="dim">of ${f0(n)}</span></span>` : '';

/* ---- career NFL memo for the index ---- */
let _cnfl = null;
function careerNfl(){
  if(_cnfl) return _cnfl;
  _cnfl = new Map();
  E.seasonRows.forEach(r=>{
    const pi=r[SCOL.p];
    let c=_cnfl.get(pi);
    if(!c){ c={g:0,fp:0,xfp2:0,fpoe2:0,s0:r[SCOL.s],s1:r[SCOL.s],tm:r[SCOL.tm]}; _cnfl.set(pi,c); }
    c.g+=r[SCOL.g]||0; c.fp+=r[SCOL.fp]||0; c.xfp2+=r[SCOL.xfp2]||0; c.fpoe2+=r[SCOL.fpoe2]||0;
    if(r[SCOL.s]>=c.s1){ c.s1=r[SCOL.s]; c.tm=r[SCOL.tm]||c.tm; }
    if(r[SCOL.s]<c.s0) c.s0=r[SCOL.s];
  });
  return _cnfl;
}

/* ---------------- PLAYERS INDEX ---------------- */
let playerQ = '', playerPos = '', playerScope = 'affl', playerSort = 'affl';
function playersView(){
  const posChips = ['QB','RB','WR','TE','K','D/ST'].map(p=>`<span class="chip ${playerPos===p?'on':''}" data-pos="${p}">${p}</span>`).join('');
  const sorts = playerScope==='affl'
    ? [['affl','AFFL pts'],['wks','Custody weeks'],['starts','Starts'],['usd','Auction $ lifetime'],['nfp','NFL FP'],['fpoe','NFL FPOE']]
    : [['nfp','NFL FP'],['fppg','FP / game'],['xfp','xFP'],['fpoe','FPOE'],['g','Games']];
  app.innerHTML = `<div class="wrap"><div class="hero"><div class="kicker">${f0(S.players.length)} AFFL identities · ${f0(E.players.length)} players in the NFL pool</div>
  ...
  const draw = ()=>{
    ...
    if(playerScope==='affl'){
      const lr = leagueRanks();
      const rows = S.players
        .filter(p => (!q || (p.name||'').toLowerCase().includes(q)) && (!playerPos || p.pos===playerPos))
        .map(p=>{
          const wks = p.stints.reduce((a,s)=>a+s.weeks,0), sts = p.stints.reduce((a,s)=>a+s.starts,0);
          const c = EIDX[p.eid]!=null ? careerNfl().get(EIDX[p.eid]) : null;
          const d = lr.dr[p.eid];
          return {p, wks, sts, affl:p.afflPts||0, usd:d?d.usd:0, nfp:c?c.fp:0, fpoe:c?c.fpoe2:0,
                  fids:[...new Set(p.stints.map(s=>s.fid))]};
        });
      const key = {affl:r=>r.affl, wks:r=>r.wks, starts:r=>r.sts, usd:r=>r.usd, nfp:r=>r.nfp, fpoe:r=>r.p.pos==='K'?-Infinity:r.fpoe}[playerSort]||(r=>r.affl);
      ...
      cn.forEach((c,pi)=>{
        ...
        if(playerPos && EPOS[pi]!==playerPos) return;
        rows.push({pi, c, ro:(E.players[pi]||[])[5]===1});
      });
      const key = {nfp:r=>r.c.fp, fppg:r=>r.c.g?r.c.fp/r.c.g:0, xfp:r=>EPOS[r.pi]==='K'?-Infinity:r.c.xfp2, fpoe:r=>EPOS[r.pi]==='K'?-Infinity:r.c.fpoe2, g:r=>r.c.g}[playerSort]||(r=>r.c.fp);
      rows.sort((a,b)=>key(b)-key(a));
      const list = rows.slice(0,300);
      $('#plist').innerHTML = `<div class="tblwrap"><table class="tbl"><thead><tr>
        <th class="l">#</th><th class="l">Player</th><th class="l">Pos</th><th class="l">Years</th><th class="l">Team</th>
        <th class="sortable">G</th><th class="sortable">FP</th><th class="sortable">FP/G</th>
        <th class="sortable">xFP</th><th class="sortable">FPOE</th><th class="l">AFFL</th></tr></thead><tbody>
        ${list.map((r,i)=>{
          const p = E.players[r.pi]||[];
          const hs = p[4] || (r.ro&&p[0]>0?'https://a.espncdn.com/i/headshots/nfl/players/full/'+p[0]+'.png':null);
          return `<tr class="click" data-href="${plRoute(r.pi)}">
          <td class="l dim">${i+1}</td>
          <td class="l"><span style="display:inline-flex;align-items:center;gap:9px">${hs?`<img class="headshot" loading="lazy" src="${esc(hs)}" alt="" onerror="this.style.display='none'">`:''}<a href="${plRoute(r.pi)}">${esc(p[1]||'')}</a></span></td>
          <td class="l pos">${esc(EPOS[r.pi]||'')}</td>
          <td class="l dim small">${r.c.s0===r.c.s1?r.c.s0:r.c.s0+'–'+r.c.s1}</td><td class="l dim small">${esc(r.c.tm||'')}</td>
          <td data-v="${r.c.g}">${r.c.g}</td><td data-v="${r.c.fp}"><b>${f1(r.c.fp)}</b></td>
          <td data-v="${r.c.g?r.c.fp/r.c.g:0}">${f1(r.c.g?r.c.fp/r.c.g:null)}</td>
          <td data-v="${p[2]==='K'?-1e18:r.c.xfp2}">${p[2]==='K'?'·':f1(r.c.xfp2)}</td>
          <td data-v="${p[2]==='K'?-1e18:r.c.fpoe2}" class="${p[2]==='K'?'':cls(r.c.fpoe2)}">${p[2]==='K'?'·':signed(r.c.fpoe2,f1)}</td>
          <td class="l">${r.ro?'<span class="badge green" title="has AFFL custody history">AFFL</span>':'<span class="badge" title="never AFFL-rostered">never held</span>'}</td></tr>`;}).join('')}
        </tbody></table></div>
        <p class="dim small" style="margin-top:8px">Showing ${list.length} of ${f0(rows.length)} QB/RB/WR/TE/K careers 2014–${LAST} (regular season). D/ST exists only as AFFL custody — see the AFFL scope.</p>`;
    }
    bindRows(); sortableTable($('#plist'));
  };
  $('#pq').addEventListener('input', e=>{ playerQ=e.target.value; draw(); });
  $('#psort').addEventListener('change', e=>{ playerSort=e.target.value; draw(); });
  app.querySelectorAll('[data-pos]').forEach(c=>c.onclick=()=>{ playerPos = playerPos===c.dataset.pos?'':c.dataset.pos; playersView(); });
  app.querySelectorAll('[data-scope]').forEach(c=>c.onclick=()=>{
    if(playerScope!==c.dataset.scope){ playerScope=c.dataset.scope; playerSort = playerScope==='affl'?'affl':'nfp'; playersView(); } });
  draw();
}

/* ---------------- PLAYER PAGE ---------------- */
function custodyMapHtml(pi){
  const C = COL, bySeason = {};
...
function resumeHtml(p, pi){
  const lr = leagueRanks();
  const wks = p.stints.reduce((a,s)=>a+s.weeks,0), sts = p.stints.reduce((a,s)=>a+s.starts,0);
  const seasonsHeld = new Set(p.stints.map(s=>s.s)).size;
  const frs = new Set(p.stints.map(s=>s.fid)).size;
  const d = lr.dr[p.eid], trN = lr.tr[p.eid]||0;
...
function seasonTableHtml(pi, pos, p){
  const rows = (pctPools().rowsByPi.get(pi)||[]).slice().sort((a,b)=>a[SCOL.s]-b[SCOL.s]);
  if(!rows.length) return '';
  const custByY = {};
  (p?p.stints:[]).forEach(st=>{ const o=custByY[st.s]=custByY[st.s]||{w:0,fids:new Set()}; o.w+=st.weeks; o.fids.add(st.fid); });
  const isQB=pos==='QB', isK=pos==='K', isRec=pos==='WR'||pos==='TE', isRB=pos==='RB';
  const head = `<tr><th class="l">Year</th><th class="l">Team</th><th>G</th>
    ${isQB?'<th>Cmp</th><th>Att</th><th>PaYd</th><th>PaTD</th><th>INT</th><th>Sk</th><th>Car</th><th>RuYd</th><th>RuTD</th>':''}
    ${isRB?'<th>Car</th><th>RuYd</th><th>YPC</th><th>RuTD</th><th>Tgt</th><th>Rec</th><th>ReYd</th><th>ReTD</th>':''}
    ${isRec?'<th>Tgt</th><th>Rec</th><th>ReYd</th><th>ReTD</th><th title="average depth of target">aDOT</th><th title="share of team targets (weekly avg)">Tgt%</th><th>Car</th><th>RuYd</th>':''}
    ${isK?'<th>FGM</th><th>FGA</th><th>FG%</th><th>XPM</th><th>XPA</th>':''}
    ${!isK?'<th title="fumbles lost">FmL</th>':''}
    <th>FP</th><th>FP/G</th>${!isK?'<th title="Expected FP, xfp_v2 (ffopportunity canon, AFFL-rescored)">xFP</th><th title="FP over expected, v2">FPOE</th><th title="opponent-adjusted FPOE (adjfac_v1)">aFPOE</th>':''}
    <th class="l" title="AFFL custody that season">AFFL</th></tr>`;
  const body = rows.map(r=>{
    const y=r[SCOL.s], cu=custByY[y];
    const g=r[SCOL.g]||0;
    return `<tr><td class="l num"><b>${y}</b></td><td class="l">${esc(r[SCOL.tm]||'')}</td><td>${g}</td>
    ${isQB?`<td>${f0(r[SCOL.cmp])}</td><td>${f0(r[SCOL.att])}</td><td>${f0(r[SCOL.pyd])}</td><td>${f0(r[SCOL.ptd])}</td><td>${f0(r[SCOL.pint])}</td><td>${f0(r[SCOL.sk])}</td><td>${f0(r[SCOL.car])}</td><td>${f0(r[SCOL.ryd])}</td><td>${f0(r[SCOL.rtd])}</td>`:''}
    ${isRB?`<td>${f0(r[SCOL.car])}</td><td>${f0(r[SCOL.ryd])}</td><td>${r[SCOL.car]?f2(r[SCOL.ryd]/r[SCOL.car]):'·'}</td><td>${f0(r[SCOL.rtd])}</td><td>${f0(r[SCOL.tgt])}</td><td>${f0(r[SCOL.rec])}</td><td>${f0(r[SCOL.recyd])}</td><td>${f0(r[SCOL.rectd])}</td>`:''}
    ${isRec?`<td>${f0(r[SCOL.tgt])}</td><td>${f0(r[SCOL.rec])}</td><td>${f0(r[SCOL.recyd])}</td><td>${f0(r[SCOL.rectd])}</td><td>${f1(r[SCOL.adot])}</td><td>${r[SCOL.tshare]!=null?pct1(r[SCOL.tshare]):'·'}</td><td>${f0(r[SCOL.car])}</td><td>${f0(r[SCOL.ryd])}</td>`:''}
    ${isK?`<td>${f0(r[SCOL.fgm])}</td><td>${f0(r[SCOL.fga])}</td><td>${r[SCOL.fga]?pct1(r[SCOL.fgm]/r[SCOL.fga]):'·'}</td><td>${f0(r[SCOL.xpm])}</td><td>${f0(r[SCOL.xpa])}</td>`:''}
    ${!isK?`<td>${f0(r[SCOL.fuml])}</td>`:''}
    <td><b>${f1(r[SCOL.fp])}</b></td><td>${f1(g?(r[SCOL.fp]||0)/g:null)}</td>
    ${!isK?`<td>${f1(r[SCOL.xfp2])}</td><td class="${cls(r[SCOL.fpoe2])}">${signed(r[SCOL.fpoe2],f1)}</td><td class="${cls(r[SCOL.afpoe2])}">${signed(r[SCOL.afpoe2],f1)}</td>`:''}
    <td class="l">${cu?[...cu.fids].map(f=>logoHtml(f,'',y)).join(' ')+` <span class="dim small">${cu.w}w</span>`
      :(y<2018?'<span class="dim small" title="pre-2018: bench custody is unobservable — a rostered-but-never-started season leaves no trace">·</span>':'<span class="dim small">FA</span>')}</td></tr>`;
  }).join('');
  return `<h2 class="sect">Season by season <span class="sub">NFL regular season · nflverse weekly stats · std non-PPR</span></h2>
  <div class="tblwrap"><table class="tbl"><thead>${head}</thead><tbody>${body}</tbody></table></div>`;
}

/* weekly game log table for one season */
function logTableHtml(logs, s, pos){
  const rows = logs.filter(r=>r.s===s);
  if(!rows.length) return '<p class="dim">No logged weeks.</p>';
  const isQB=pos==='QB', isK=pos==='K', isRec=pos==='WR'||pos==='TE', isRB=pos==='RB';
  const affl = r=>{
    if(!r.cust) return '<span class="dim small">FA</span>';
    const st = r.cust.ev? '<span class="badge" title="pre-2018: lineup slot unobservable">held</span>'
      : r.cust.st? '<span class="badge green">ST</span>'
      : (r.fp!=null&&r.fp>=15? '<span class="badge red" title="left on the bench for a big game">BN</span>':'<span class="badge">BN</span>');
    return `<span style="display:inline-flex;align-items:center;gap:6px">${logoHtml(r.cust.fid,'',r.s)}${st}</span>`;
  };
  const head = `<tr><th>Wk</th><th class="l">Tm</th><th class="l">Opp</th><th class="l" title="AFFL custody: franchise + started/bench">AFFL</th>
    ${isQB?'<th>Cmp</th><th>Att</th><th>PaYd</th><th>PaTD</th><th>INT</th><th>Sk</th><th>Car</th><th>RuYd</th><th>RuTD</th>':''}
    ${isRB?'<th>Car</th><th>RuYd</th><th>RuTD</th><th>Tgt</th><th>Rec</th><th>ReYd</th><th>ReTD</th>':''}
    ${isRec?'<th>Tgt</th><th>Rec</th><th>ReYd</th><th>ReTD</th><th>Car</th><th>RuYd</th>':''}
    ${isK?'<th>FGM</th><th>FGA</th><th>XPM</th><th>XPA</th>':''}
    ${!isK?'<th title="fumbles lost">FmL</th>':''}
    <th>FP</th>${!isK?'<th>xFP</th><th>FPOE</th>':''}</tr>`;
  const body = rows.map(r=>{
    if(r.dnp) return `<tr class="dnp"><td>${r.w}</td><td class="l" colspan="2"><span class="dim">no NFL game — bye / inactive</span></td>
      <td class="l">${affl(r)}</td><td class="l dim small" colspan="${(isQB?9:isRB?7:isRec?6:isK?4:0)+(isK?0:1)+(isK?1:3)}">${r.cust&&r.cust.affl!=null?'AFFL applied: '+f1(r.cust.affl)+' pts':''}</td></tr>`;
    return `<tr><td><b>${r.w}</b></td><td class="l dim small">${esc(r.tm||'')}</td><td class="l small">${oppCell(r.s, r.opp, pos)}</td>
    <td class="l">${affl(r)}</td>
    ${isQB?`<td>${f0(r.cmp)}</td><td>${f0(r.att)}</td><td>${f0(r.pyd)}</td><td>${f0(r.ptd)}</td><td>${f0(r.int)}</td><td>${f0(r.sk)}</td><td>${f0(r.car)}</td><td>${f0(r.ryd)}</td><td>${f0(r.rtd)}</td>`:''}
    ${isRB?`<td>${f0(r.car)}</td><td>${f0(r.ryd)}</td><td>${f0(r.rtd)}</td><td>${f0(r.tgt)}</td><td>${f0(r.rec)}</td><td>${f0(r.recyd)}</td><td>${f0(r.rectd)}</td>`:''}
    ${isRec?`<td>${f0(r.tgt)}</td><td>${f0(r.rec)}</td><td>${f0(r.recyd)}</td><td>${f0(r.rectd)}</td><td>${f0(r.car)}</td><td>${f0(r.ryd)}</td>`:''}
    ${isK?`<td>${f0(r.fgm)}</td><td>${f0(r.fga)}</td><td>${f0(r.xpm)}</td><td>${f0(r.xpa)}</td>`:''}
    ${!isK?`<td>${r.fuml?`<span class="neg">${r.fuml}</span>`:'0'}</td>`:''}
    <td><b>${f1(r.fp)}</b></td>${!isK?`<td class="dim">${f1(r.xfp)}</td><td class="${cls(r.fpoe)}">${signed(r.fpoe,f1)}</td>`:''}</tr>`;
  }).join('');
  return `<div class="tblwrap"><table class="tbl"><thead>${head}</thead><tbody>${body}</tbody></table></div>`;
}

function drawSeasonArc(pi, eid){
  const rows = (pctPools().rowsByPi.get(pi)||[]).slice().sort((a,b)=>a[SCOL.s]-b[SCOL.s]);
  if(rows.length<2) return;
  const isK=EPOS[pi]==='K';
  const ys = rows.map(r=>r[SCOL.s]);
  const fpg = rows.map(r=>r[SCOL.g]?+((r[SCOL.fp]||0)/r[SCOL.g]).toFixed(2):null);
  const xfpg = isK?[]:rows.map(r=>r[SCOL.g]&&r[SCOL.xfp2]?+((r[SCOL.xfp2]||0)/r[SCOL.g]).toFixed(2):null);
  const bids = ys.map(y=>{ const d=S.drafts.find(d=>d.eid===eid&&d.s===y&&d.bid>0); return d?d.bid:null; });
  mkChart('plarc', {
    tooltip: ECTT({}),
    legend: {top:0, textStyle:{color:'#9fb0c8', fontSize:11}},
    grid: {left:50, right:52, top:34, bottom:26},
    xAxis: ECX({data:ys}),
    yAxis: [ECY(), ECY({splitLine:{show:false}, axisLabel:{formatter:'${value}', color:'#8fa2bd', fontFamily:'IBM Plex Mono', fontSize:10.5}})],
    series: [
      {name:'Auction $ paid', type:'bar', yAxisIndex:1, data:bids, barWidth:'42%',
       itemStyle:{color:'rgba(255,196,0,.32)', borderColor:'#ffc400', borderWidth:1}},
      {name:'FP / game', type:'line', data:fpg, smooth:.3, symbol:'circle', symbolSize:6,
       lineStyle:{width:2.5, color:'#00a2ff'}, itemStyle:{color:'#00a2ff'}, connectNulls:true},
      ...(isK?[]:[{name:'xFP / game', type:'line', data:xfpg, smooth:.3, symbol:'none',
       lineStyle:{width:1.6, type:'dashed', color:'#c8ff00'}, itemStyle:{color:'#c8ff00'}, connectNulls:true}]),
    ],
  });
}
function drawWeeklyBeat(pi){
  const logs = playerLogs(pi).filter(r=>!r.dnp);
  if(logs.length<5) return;
  const isK=EPOS[pi]==='K';
  const xs = logs.map(r=>r.s+' W'+r.w);
  const marks = []; xs.forEach((x,i)=>{ if(i>0 && logs[i].s!==logs[i-1].s) marks.push({xAxis:i-0.5}); });
  mkChart('plbeat', {
    tooltip: ECTT({formatter: ps=>{
      const i = ps[0].dataIndex, r = logs[i];
      const cu = r.cust? `${esc(histName(r.s, r.cust.fid)||'')} · ${r.cust.ev?'held':r.cust.st?'started':'benched'}` : 'not AFFL-rostered';
      return `<b>${r.s} Week ${r.w}</b> · ${esc(r.tm||'')}${r.opp?' v '+esc(r.opp):''}<br>FP <b>${f1(r.fp)}</b>${isK?'':' · xFP '+f1(r.xfp)+' · FPOE '+signed(r.fpoe,f1)}<br><span style="color:#8fa2bd">${cu}</span>`; }}),
    grid: {left:46, right:16, top:26, bottom:30},
    xAxis: ECX({data:xs, axisLabel:{interval:(i,v)=>v.endsWith(' W1'), formatter:v=>v.slice(0,4), fontSize:9.5}}),
    yAxis: ECY(),
    series: [
      {name:'FP', type:'bar', barWidth:'62%',
       data: logs.map(r=>({value:r.fp, itemStyle:{color: r.cust? (r.cust.st||r.cust.ev? fColor(r.cust.fid) : '#39445e') : '#20293c',
         borderColor: r.cust&&!r.cust.st&&!r.cust.ev&&(r.fp||0)>=15? '#ff4d5e':'transparent', borderWidth: 1.4}})),
       markLine:{silent:true, symbol:'none', label:{show:false}, lineStyle:{color:'#26314a', width:1}, data:marks}},
      ...(isK?[]:[{name:'xFP', type:'line', data:logs.map(r=>r.xfp), symbol:'none', lineStyle:{width:1.4, color:'#c8ff00', opacity:.85}, z:3}]),
    ],
  });
}
...
function playerView(eid){
  ...
  const pi = EIDX[eid];
  const pos = p.pos;
  const hs = headshotUrl(p);
  const cn = pi!=null? careerNfl().get(pi) : null;
  const acc = teamAccent(p.team || (cn&&cn.tm));
  const {html:strip, legend} = pi!=null? custodyMapHtml(pi) : {html:'',legend:''};
  ...
  const myTrades = S.trades.filter(t=>Object.values(t.sides).some(l=>l.includes(eid))).sort((a,b)=>b.s-a.s||b.week-a.week);
  const hasNfl = pi!=null && (pctPools().rowsByPi.get(pi)||[]).length>0;
  const seasonsWith = hasNfl? [...new Set((pctPools().rowsByPi.get(pi)||[]).map(r=>r[SCOL.s]))].sort((a,b)=>b-a) : [];

  app.innerHTML = `<div class="wrap">
    <div class="phero" style="background:linear-gradient(120deg, ${acc}26, var(--card) 58%);border-color:${acc}55">
      ${hs?`<img class="hshot" src="${esc(hs)}" alt="" onerror="this.style.display='none'">`:''}
      <div style="min-width:0;flex:1">
        <div class="kicker">${esc(pos||'')} ${p.team?'· '+esc(p.team):''} ${p.college?'· '+esc(p.college):''} ${p.rookie?'· rookie '+p.rookie:''}
          ${p.draftOnly?' <span class="badge yellow">draft-only identity</span>':''}${p.dst?' <span class="badge blue">team defense</span>':''}</div>
    ...
    <div class="grid g2" style="margin-top:22px">
      ...
      ${stintRows?`<div class="tblwrap"><table class="tbl"><thead><tr><th class="l">Year</th><th class="l">Franchise</th><th>Span</th><th>Wks</th><th>Starts</th><th>Pts</th><th>PAR</th><th class="l">Acq</th></tr></thead><tbody>${stintRows}</tbody></table></div>`:'<p class="dim">No observable custody weeks.</p>'}</div>
    ...
    ${myTrades.length?`<h2 class="sect">Trades involving ${esc(p.name)} <span class="sub">value = AFFL points rest-of-season (trade_v1)</span></h2>
    <div class="grid g2">${myTrades.map(t=>tradeCard(t)).join('')}</div>`:''}
    ${hasNfl?`
    <h2 class="sect">NFL profile <span class="sub">percentile vs qualified ${esc(pos)} peers · pick a scope</span></h2>
    <div class="presets" id="pctchips">${['career',...seasonsWith].map((s,i)=>`<span class="chip ${i===(seasonsWith.length?1:0)?'on lime':''}" data-pct="${s}">${s==='career'?'Career':s}</span>`).join('')}</div>
    <div class="grid g2" style="margin-top:10px"><div class="card" id="pctwrap"></div>
    ...
    <div class="presets" id="glchips">${seasonsWith.map((s,i)=>`<span class="chip ${i===0?'on':''}" data-gls="${s}">${s}</span>`).join('')}</div>
    <div id="glwrap"><p class="dim small" style="padding:16px 4px">loading game logs…</p></div>`
    : p.dst? `<h2 class="sect">Weekly custody log <span class="sub">D/ST production exists at the league grain — AFFL applied points while rostered</span></h2>
    <div class="presets" id="glchips"></div><div id="glwrap"></div>`
    : ''}
    ${pi!=null?`<div class="presets" style="margin-top:18px">
      <a class="chip" href="#/explore?q=${encState(Object.assign(defState(),{pi,gr:'weeks',sc:'rostered',ms:['affl','fp','tgt','car'],sort:'affl',lim:100}))}">▦ Every custody week in Explore</a>
      ${hasNfl?`<a class="chip" href="#/compare?p=${pi}">⇄ Compare with…</a>`:''}
    </div>`:''}
  </div>`;

  if(hasNfl){
    const drawPct = scope=>{
      const rows = pctPools().rowsByPi.get(pi)||[];
      let agg;
      if(scope==='career'){ agg=aggInit(); rows.forEach(r=>aggAdd(agg,r)); }
      else { agg=aggInit(); rows.filter(r=>r[SCOL.s]===+scope).forEach(r=>aggAdd(agg,r)); }
      $('#pctwrap').innerHTML = `<div class="kicker" style="margin-bottom:10px">${scope==='career'?'Career profile':scope+' profile'} <span class="dim">· ${agg.g||0} games</span></div>`
        + pctBarsHtml(pos, agg, scope==='career'?'career':String(+scope), scope==='career');
    };
    app.querySelectorAll('[data-pct]').forEach(c=>c.onclick=()=>{
      app.querySelectorAll('[data-pct]').forEach(x=>x.classList.remove('on','lime'));
      c.classList.add('on','lime'); drawPct(c.dataset.pct); });
    drawPct(seasonsWith.length? String(seasonsWith[0]) : 'career');
    drawSeasonArc(pi, eid);
    const drawLogs = ()=>{
      const logs = playerLogs(pi);
      const seasons = [...new Set(logs.map(r=>r.s))].sort((a,b)=>b-a);
      if(!seasons.length){ $('#glwrap').innerHTML='<p class="dim">No logged weeks.</p>'; return; }
      if(!glSeason || !seasons.includes(glSeason)) glSeason = seasons[0];
      $('#glchips').innerHTML = seasons.map(s=>`<span class="chip ${s===glSeason?'on':''}" data-gls="${s}">${s}</span>`).join('');
      $('#glwrap').innerHTML = logTableHtml(logs, glSeason, pos)
        + `<p class="dim small" style="margin-top:8px">FP is nflverse std non-PPR (fumbles included). ${pos==='K'?'Kicking is outside xfp_v2.':'xFP/FPOE are xfp_v2 (no fumble term).'} ${['QB','RB','WR','TE'].includes(pos)?'● on the opponent marks defense-vs-'+esc(pos)+' generosity that season. ':''}Pre-2018 AFFL columns show starters only — bench custody is unobservable.</p>`;
      $('#glchips').querySelectorAll('[data-gls]').forEach(c=>c.onclick=()=>{ glSeason=+c.dataset.gls; drawLogs(); });
      drawWeeklyBeat(pi);
    };
    ensureGL(ok=>{
      if(location.hash.indexOf('#/p/'+eid)!==0) return;   // navigated away mid-fetch
      if(!ok){ $('#glwrap').innerHTML = '<div class="notice">Game logs could not be fetched — custody weeks are still queryable in <a href="#/explore">Explore</a>.</div>'; drawWeeklyBeat(pi); return; }
      drawLogs();
    });
  } else if(p.dst && pi!=null){
    const C = COL;
    const mine = E.rows.filter(r=>r[C.p]===pi);
    const seasons = [...new Set(mine.map(r=>r[C.s]))].sort((a,b)=>b-a);
    if(seasons.length){
      if(!glSeason || !seasons.includes(glSeason)) glSeason = seasons[0];
      const drawDst = ()=>{
        $('#glchips').innerHTML = seasons.map(s=>`<span class="chip ${s===glSeason?'on':''}" data-gls="${s}">${s}</span>`).join('');
        const rows = mine.filter(r=>r[C.s]===glSeason).sort((a,b)=>a[C.w]-b[C.w]);
        $('#glwrap').innerHTML = `<div class="tblwrap"><table class="tbl"><thead><tr><th>Wk</th><th class="l">Rostered by</th><th class="l">Status</th><th>AFFL pts</th></tr></thead><tbody>
          ${rows.map(r=>{ const fid=E.franchises[r[C.f]][0];
            return `<tr><td><b>${r[C.w]}</b></td><td class="l">${frLink(fid, r[C.s])}</td>
            <td class="l">${r[C.ev]?'<span class="badge">held</span>':r[C.st]?'<span class="badge green">ST</span>':'<span class="badge">BN</span>'}</td>
            <td><b>${f1(r[C.affl])}</b></td></tr>`;}).join('')}</tbody></table></div>
          <p class="dim small" style="margin-top:8px">NFL box stats for team defenses are not modeled — D/ST value is scored by ESPN and tracked here at the custody grain.</p>`;
        $('#glchips').querySelectorAll('[data-gls]').forEach(c=>c.onclick=()=>{ glSeason=+c.dataset.gls; drawDst(); });
      };
      drawDst();
    }
  }
...
/* NFL-only player page (never AFFL-rostered; reached from Leaderboards) */
function nflPlayerView(pi){
  const pl = E.players[pi];
  if(!pl){ app.innerHTML='<div class="wrap"><p style="padding:40px 0">Unknown player.</p></div>'; return; }
  if(pl[0]!=null && PL[pl[0]]){ playerView(pl[0]); return; }
  const pos = EPOS[pi], nm = ENAME[pi];
  const cn = careerNfl().get(pi);
  const rows = (pctPools().rowsByPi.get(pi)||[]);
  const seasonsWith = [...new Set(rows.map(r=>r[SCOL.s]))].sort((a,b)=>b-a);
  const acc = teamAccent(cn&&cn.tm);
  app.innerHTML = `<div class="wrap">
    <div class="phero" style="background:linear-gradient(120deg, ${acc}26, var(--card) 58%);border-color:${acc}55">
      ${pl[4]?`<img class="hshot" src="${esc(pl[4])}" alt="" onerror="this.style.display='none'">`:''}
      <div style="min-width:0;flex:1">
        <div class="kicker">${esc(pos||'')} ${cn&&cn.tm?'· '+esc(cn.tm):''} · <span class="badge" style="vertical-align:1px" title="in the NFL pool for leaderboards; never on an AFFL roster">never AFFL-rostered</span></div>
        <h1 class="display" style="font-size:clamp(30px,5vw,52px)">${esc(nm)}</h1>
        <div class="statline">
          ${cn?`<div class="stat"><div class="v">${cn.s0===cn.s1?cn.s0:cn.s0+'–'+cn.s1}</div><div class="l">Seasons</div></div>
          <div class="stat"><div class="v">${f0(cn.g)}</div><div class="l">Games</div></div>
          <div class="stat"><div class="v">${f1(cn.fp)}</div><div class="l">FP (std)</div></div>
          ${pos!=='K'?`<div class="stat"><div class="v ${cls(cn.fpoe2)}">${signed(cn.fpoe2,f1)}</div><div class="l">Career FPOE</div></div>`:''}`:''}
        </div></div></div>
    <div class="notice" style="margin-top:14px">This player was never rostered in the AFFL — twelve drafts, ${f0(S.players.length)} held identities, and nobody bit. NFL production only; weekly game logs are kept for league-held players.</div>
    ${rows.length?`<h2 class="sect">NFL profile <span class="sub">percentile vs qualified ${esc(pos)} peers</span></h2>
    <div class="presets" id="pctchips">${['career',...seasonsWith].map((s,i)=>`<span class="chip ${i===(seasonsWith.length?1:0)?'on lime':''}" data-pct="${s}">${s==='career'?'Career':s}</span>`).join('')}</div>
    <div class="grid g2" style="margin-top:10px"><div class="card" id="pctwrap"></div>
      <div><div class="card chartbox" style="padding:12px 10px 6px"><div id="plarc" style="width:100%;height:252px"></div></div></div></div>
    ${seasonTableHtml(pi, pos, null)}`:''}
    <div class="presets" style="margin-top:18px"><a class="chip" href="#/compare?p=${pi}">⇄ Compare with…</a>
      <a class="chip" href="#/boards?${new URLSearchParams({m:'fp',pos:pos||''}).toString()}">▤ ${esc(pos||'')} leaderboards</a></div>
  </div>`;
  if(rows.length){
    const drawPct = scope=>{
      let agg=aggInit();
      (scope==='career'? rows : rows.filter(r=>r[SCOL.s]===+scope)).forEach(r=>aggAdd(agg,r));
      $('#pctwrap').innerHTML = `<div class="kicker" style="margin-bottom:10px">${scope==='career'?'Career profile':scope+' profile'} <span class="dim">· ${agg.g||0} games</span></div>`
        + pctBarsHtml(pos, agg, scope==='career'?'career':String(+scope), scope==='career');
    };
    app.querySelectorAll('[data-pct]').forEach(c=>c.onclick=()=>{
...
function seasonsView(){
  const cards = YEARS.slice().reverse().map(y=>{
    const sd = S.seasons[y];
    if(!sd.complete) return `<div class="card" style="cursor:pointer" onclick="location.hash='#/s/${y}'"><div class="kicker">${y}</div>
...
function seasonView(y){
  ...
  const rows = teams.map(t=>sd.complete ? `<tr class="click" data-href="#/f/${t.fid}">
    <td>${t.finalRank||'·'}</td>
    ...
    <td data-v="${t.pf||0}">${f1(t.pf)}</td><td data-v="${t.pa||0}">${f1(t.pa)}</td>
    <td>${t.seed||'·'}</td></tr>` : `<tr class="click" data-href="#/f/${t.fid}">
    <td class="l">${logoHtml(t.fid,'',y)} <a class="nm" href="#/f/${t.fid}">${esc(t.name)}</a></td>
    <td class="l dim small">${esc((F[t.fid]||{}).owners?.join(', ')||'')}</td>
    <td class="l num">${esc(t.abbrev||'')}</td></tr>`).join('');
  let bracket = '';
  if(sd.complete){
  ...
  app.innerHTML = `<div class="wrap">
    ...
    ${sd.complete?`<h2 class="sect">Final standings</h2>
    ...
    `<h2 class="sect">Registered teams</h2>
    <div class="tblwrap"><table class="tbl"><thead><tr><th class="l">Team</th><th class="l">Owner</th><th class="l">Abbrev</th></tr></thead><tbody>${rows}</tbody></table></div>`}
...
function recordsView(qs){
  ...
  app.innerHTML = `<div class="wrap"><div class="hero"><div class="kicker">The AFFL record book · regular season 2014–${LAST}</div>
    <h1 class="display">Records</h1>
    <p class="tag">Regular season only, like every other record on this site — playoff and consolation games are excluded (2014–16 playoff "games" were two-week totals, not comparable to a week).</p></div>
    <div class="grid g2">
      ...
      ${half('Biggest blowouts','<th class="l">Game</th><th>Year</th><th>Wk</th><th>Margin</th>', r.blowouts.map(gmRow).join(''))}
      ${half('Closest games','<th class="l">Game</th><th>Year</th><th>Wk</th><th>Margin</th>', r.closest.map(gmRow).join(''))}
    </div>
    <h2 class="sect">Best started player-weeks <span class="sub">regular season</span></h2>
...
function luckView(yArg){
  ...
  app.innerHTML = `<div class="wrap">
    ...
    <p class="dim small" style="margin-top:8px">Sorted by all-play win% — the closest thing to a luck-free ranking of how well each franchise actually scored. <b>Expected W</b> is all-play win% applied to games played; <b>Luck</b> is actual minus expected and sums to zero across the league. <b>Actual</b> is the official ESPN ledger: the AFFL has never tied — its four equal-score games (2014, 2015, 2016, 2018) were settled by ESPN's tiebreaker, all four to the road team, and count here as the wins and losses they officially were. Winning a game you didn't outscore anyone in is schedule luck at its purest, and it is counted as such.</p>
    ...
    <p class="dim small" style="margin-top:8px">Read across: each cell is that team's win total if it had faced the column team's actual opponent sequence, with all scores unchanged. Green means the borrowed schedule was kinder than their own. Counterfactual pairings have no ESPN tiebreaker, so an equal-score pairing counts as a tie here — in the four seasons with a real tiebreaker game, the diagonal (your own schedule) is the pure-scoreboard record and can differ from the official <b>Real</b> column by that one game.</p>
...
function methodsView(){
  ...
  <div class="card">
    ...
    <div class="mrow"><span class="k">allplay_v1</span><span>Every regular-season week, each team's score is ranked against every other team playing that week: all-play W/L/T = teams scored below / above / tied. Expected wins = all-play win% × games played; <b>luck = actual − expected</b> and sums to zero league-wide. Consolation and playoff games excluded. <b>Actual results are ESPN's official outcomes:</b> the league's four equal-score games (2014 MP3, 2015 MP11, 2016 MP11, 2018 MP5) were settled by ESPN's tiebreaker — all four to the road team — and count as official wins/losses in records, streaks and ELO. All-play and vs-median remain score-based, so those four performances score as ties there; a tiebreaker win therefore shows up, correctly, as luck.</span></div>
  ...
  <p class="dim small" style="margin-top:8px">Pre-2018, ESPN's historical matchup rosters omit players later dropped from the roster; their points (3.1% of pre-2018 scoring) remain at team level and are never re-assigned. Exact lineup slots before 2018 are NULL with an explicit “Unavailable” label — we do not infer them. Two-week playoff matchups (2014–2016) retain player points at matchup grain only. The record book and all luck/streak/ELO metrics are <b>regular season only</b>; playoff results appear in brackets and playoff W–L. One further evidence note: ESPN's 2022 finals week carries five commissioner scoring adjustments (+12.9 to +26.2) that exist in official matchup totals but not in weekly lineup scores — brackets show the official totals, and the regular-season scope keeps the unadjusted weekly numbers out of every record.</p>
...
const NO_K_MODEL = new Set(['xfp2','fpoe2','afpoe2','fpoePg','fpoe1']);
const QUICK_BOARDS = [
...
function playerCell(pi, seasonCtx){
  ...
  return `<a href="#/pn/${pi}">${nm}</a> <span class="pos">${esc(p[2]||'')}</span> <span class="badge" title="never AFFL-rostered">NFL</span>`;
}
function boardsView(qs){
  const q = new URLSearchParams(qs||'');
  const yRaw = q.has('y') ? Number(q.get('y')) : LAST;
  const y = [0,...DONE].includes(yRaw) ? yRaw : LAST;
  const m = BK[q.get('m')] ? q.get('m') : 'fpoe2';
  const bm = BK[m];
  const validPos = new Set(['QB','RB','WR','TE','K']);
  const pos = [...new Set((q.get('pos')||'').split(',').filter(p=>validPos.has(p)))];
  const scope = q.get('scope')==='ro' ? 'ro' : 'all';
  const asc = q.get('d')==='a';
  const limRaw = Number(q.get('n')||50), lim = [25,50,100,250].includes(limRaw) ? limRaw : 50;
  const rawG = q.get('g'), rawV = q.get('v');
  const ming = rawG!=null && rawG!=='' && Number.isFinite(Number(rawG)) ? Math.max(0,Number(rawG)) : (bm.ming||0);
  const mind = rawV!=null && rawV!=='' && Number.isFinite(Number(rawV)) ? Math.max(0,Number(rawV)) : (bm.mind||0);
  ...
  const dyRaw = Number(q.get('dy')||LAST);
  const dy = dyRaw===0 || dySel.includes(dyRaw) ? dyRaw : LAST;
  const dpos = dposList.includes(q.get('dpos')) ? q.get('dpos') : 'RB';
  const st = {y, m, pos:pos.join(','), scope, d:asc?'a':'', n:lim, g:ming, v:mind, dy, dpos};
  const link = o => {
    const next = Object.assign({}, st, o);
  ...
  for(const r of E.seasonRows){
    ...
    if(NO_K_MODEL.has(m) && EPOS[pi]==='K') continue;
  ...
  // ---- defense boards ----
  const dSource = DB_.filter(r=>r[DBC.pos]===dpos && (dy===0 || r[DBC.s]===dy));
  let dAgg;
  if(dy===0){
    const byDef = new Map();
    dSource.forEach(r=>{
      const g=Number(r[DBC.g])||0, k=r[DBC.def];
      let x=byDef.get(k); if(!x){x={def:k,g:0,std:0,afp2:0,xfp2:0};byDef.set(k,x);}
      x.g+=g; x.std+=(Number(r[DBC.stdpg])||0)*g; x.afp2+=(Number(r[DBC.afp2pg])||0)*g; x.xfp2+=(Number(r[DBC.xfp2pg])||0)*g;
    });
    const leagueG=[...byDef.values()].reduce((a,x)=>a+x.g,0);
    const leagueAfp=[...byDef.values()].reduce((a,x)=>a+x.afp2,0);
    const leagueRate=leagueG?leagueAfp/leagueG:null;
    dAgg=[...byDef.values()].map(x=>({def:x.def,g:x.g,stdpg:x.g?x.std/x.g:null,afp2pg:x.g?x.afp2/x.g:null,xfp2pg:x.g?x.xfp2/x.g:null,idx:leagueRate&&x.g?(x.afp2/x.g)/leagueRate:null}));
  }else{
    dAgg=dSource.map(r=>({def:r[DBC.def],g:r[DBC.g],stdpg:r[DBC.stdpg],afp2pg:r[DBC.afp2pg],xfp2pg:r[DBC.xfp2pg],idx:r[DBC.idx]}));
  }
  const dRows = dAgg.sort((a,b)=>(b.stdpg||0)-(a.stdpg||0)).map((r,i)=>{
      const idx = r.idx;
      ...
      return `<tr><td class="l dim">${i+1}</td><td class="l"><b>${esc(r.def)}</b></td><td>${r.g}</td>
        <td data-v="${r.stdpg}"><b>${f1(r.stdpg)}</b></td>
        <td>${f1(r.afp2pg)}</td><td>${f1(r.xfp2pg)}</td>
  ...
  app.innerHTML = `<div class="wrap">
    ...
    <div class="xrow"><span class="lbl">Season</span><a class="chip ${dy===0?'on':''}" href="${link({dy:0})}">All Seasons</a>${dySel.map(x=>`<a class="chip ${dy===x?'on':''}" href="${link({dy:x})}">${x}</a>`).join('')}</div>
    ...
    <p class="dim small" style="margin-top:8px">Most generous defenses first. <b>Index</b> is v2-scope points allowed per game vs the league average for that ${dy===0?'position across 2014–'+LAST:'position-season'} (1.00 = league); it is the aggregate cousin of the leave-one-week-out factor used in adjusted FPOE.</p>
  </div>`;
  $('#bmeasure').onchange = e=>{ location.hash = link({m:e.target.value, g:null, v:null}); };
...
function compareView(qs){
  const q = new URLSearchParams(qs||'');
  const sel = [...new Set((q.get('p')||'').split(',').filter(Boolean).map(Number).filter(i=>Number.isInteger(i)&&E.players[i]&&E.seasonRows.some(r=>r[SCOL.p]===i)))].slice(0,4);
  ...
  const vOf = r => { if(NO_K_MODEL.has(m)&&EPOS[r[SCOL.p]]==='K') return null; const v=bm.num(r); if(v==null||isNaN(v)) return null; return bm.den? (bm.den(r)? v/bm.den(r):null) : v; };
  const careers = sel.map(pi=>{
    const rs = rowsBy.get(pi)||[];
    const c = {g:0,fp:0,xfp2:0,fpoe2:0,hasXfp:false,hasFpoe:false,num:0,den:0,has:false};
    rs.forEach(r=>{ c.g+=r[SCOL.g]||0; c.fp+=r[SCOL.fp]||0;
      if(EPOS[pi]!=='K'&&r[SCOL.xfp2]!=null){c.xfp2+=r[SCOL.xfp2];c.hasXfp=true;}
      if(EPOS[pi]!=='K'&&r[SCOL.fpoe2]!=null){c.fpoe2+=r[SCOL.fpoe2];c.hasFpoe=true;}
      if(!NO_K_MODEL.has(m)||EPOS[pi]!=='K'){const v=bm.num(r); if(v!=null&&!isNaN(v)){c.num+=v;c.has=true;} if(bm.den){const d=bm.den(r); if(d!=null&&!isNaN(d))c.den+=d;}}
    });
    c.val = !c.has? null : bm.den? (c.den? c.num/c.den:null) : c.num;
    return c;
  });
  const radarEligible = sel.filter((pi,i)=>EPOS[pi]!=='K' && careers[i].g>=16).length;
  const allVals = [];
  ...
  const cards = sel.map((pi,ci)=>{
    ...
    return `<div class="card" style="position:relative">
      ...
      <div style="display:flex;gap:12px;align-items:center">
        ...
        <div><div style="font-family:'Barlow Condensed';font-weight:700;font-size:20px"><a href="${plRoute(pi)}">${esc(p[1])}</a></div>
      ...
      <div class="statline" style="margin-top:10px">
        ...
        <div class="stat"><div class="v">${c.hasXfp?f1(c.xfp2):'·'}</div><div class="l">xFP</div></div>
        <div class="stat"><div class="v ${c.hasFpoe?cls(c.fpoe2):''}">${c.hasFpoe?signed(c.fpoe2,f1):'·'}</div><div class="l">FPOE</div></div>
  ...
  app.innerHTML = `<div class="wrap">
    ...
    <p class="dim small" style="margin-top:8px">Season grain, regular season only, 2014–${LAST}. Expected FP is xfp_v2; AFFL custody totals appear on each player's card when they have league history.${sel.some(pi=>EPOS[pi]==='K')?' Kicking is outside xfp_v2, so kicker xFP/FPOE values are unavailable rather than zero.':''}</p>`:''}
  </div>`;

  if(radarEligible>=2) drawCmpRadar(sel, rowsBy);
...
function defState(){ return {sc:'started', gr:'player', s0:2014, s1:LAST, w0:1, w1:18, f:[], pos:[], pq:'', pi:null,
...
const EVER_GRAINS = new Set(['player','playerSeason','season']);
function normalizeExploreState(st){
  const out=Object.assign(defState(),st||{});
  out.sc=['started','rostered','bench','ever'].includes(out.sc)?out.sc:'started';
  out.gr=GRAINS.some(g=>g[0]===out.gr)?out.gr:'player';
  out.f=Array.isArray(out.f)?out.f.filter(fid=>F[fid]):[];
  out.pos=Array.isArray(out.pos)?out.pos.filter(p=>['QB','RB','WR','TE','K','D/ST'].includes(p)):[];
  out.ms=Array.isArray(out.ms)?out.ms.filter(k=>MK[k]):[];
  out.pi=out.pi!=null&&Number.isInteger(Number(out.pi))&&E.players[Number(out.pi)]?Number(out.pi):null;
  out.s0=DONE.includes(Number(out.s0))?Number(out.s0):DONE[0];
  out.s1=DONE.includes(Number(out.s1))?Number(out.s1):LAST;
  if(out.s0>out.s1) [out.s0,out.s1]=[out.s1,out.s0];
  out.w0=Math.max(1,Math.min(18,Number(out.w0)||1)); out.w1=Math.max(out.w0,Math.min(18,Number(out.w1)||18));
  out.lim=[25,50,100,250,1000].includes(Number(out.lim))?Number(out.lim):50;
  out.md=Math.max(0,Number(out.md)||0); out.dir=Number(out.dir)===1?1:-1;
  if(out.sc==='ever'){
    if(!EVER_GRAINS.has(out.gr)) out.gr='playerSeason';
    out.f=[]; out.w0=1; out.w1=18; out.pos=out.pos.filter(p=>p!=='D/ST');
    out.ms=out.ms.filter(k=>MK[k]&&MK[k].everOk);
    if(!out.ms.length) out.ms=['fp','xfp2','fpoe2'];
  }else if(!out.ms.length) out.ms=defState().ms;
  if(!out.ms.includes(out.sort)) out.sort=out.ms[0];
  return out;
}
...
const M = [
 ...
 {k:'xfp2', l:'xFP (v2)', num:r=>r[R.xfp2], fmt:f1, everOk:true, skillOnly:true, sNum:r=>r[SCOL.xfp2]},
 {k:'fpoe2', l:'FPOE (v2)', num:r=>r[R.fpoe2], fmt:v=>signed(v,f1), everOk:true, skillOnly:true, sNum:r=>r[SCOL.fpoe2]},
 {k:'xfp', l:'xFP v1 (legacy)', num:r=>r[R.xfp], fmt:f1, everOk:true, skillOnly:true, sNum:r=>r[SCOL.xfp]},
 {k:'fpoe', l:'FPOE v1 (legacy)', num:r=>r[R.fpoe], fmt:v=>signed(v,f1), everOk:true, skillOnly:true, sNum:r=>r[SCOL.fpoe]},
...
const PRESETS = [
 ...
 {id:'careers', icon:'👑', label:'Custody careers', blurb:'Career AFFL value by player',
  state:{sc:'rostered', gr:'player', ms:['weeks','starts','affl','fp','fpoe2'], sort:'affl'}},
];
...
function exploreView(qs){
  ...
  app.innerHTML = `<div class="wrap">
    ...
    <div class="xbar">
      ...
      <div class="xrow"><span class="lbl">Seasons</span>
        ...
        <input id="xw0" type="number" min="1" max="18" value="${X.w0}" style="width:60px" ${X.sc==='ever'?'disabled title="NFL-season scope has no week filter"':''}>
        <span class="dim">→</span>
        <input id="xw1" type="number" min="1" max="18" value="${X.w1}" style="width:60px" ${X.sc==='ever'?'disabled title="NFL-season scope has no week filter"':''}>
  ...
  // bindings
  app.querySelectorAll('[data-sc]').forEach(c=>c.onclick=()=>{ X.sc=c.dataset.sc; if(X.sc==='bench'&&X.s0<2018) X.s0=2018; X=normalizeExploreState(X); syncHash(); });
  ...
  $('#xpq').onchange=e=>{X.pi=null; X.pq=e.target.value; syncHash();};
...
function syncHash(){ X=normalizeExploreState(X); location.hash = '#/explore?q='+encState(X); }
...
function filterRows(){
  ...
  for(let i=0;i<E.rows.length;i++){
    ...
    if(X.pi!=null && r[C.p]!==X.pi) continue;
...
function aggregate(){
  ...
  for(const i of idx){
    ...
    for(const m of active){
      if(m.skillOnly&&['K','D/ST'].includes(EPOS[r[COL.p]])) continue;
      const v = m.num(r);
  ...
  const val = (g,m)=>{
    const has=Object.prototype.hasOwnProperty.call(g.sums,m.k);
    return m.den ? (has&&g.dens[m.k]?g.sums[m.k]/g.dens[m.k]:null) : (has?g.sums[m.k]:null);
  };
...
function aggregateEver(){
  ...
  const rows = E.seasonRows.filter(r => r[SCOL.s]>=X.s0 && r[SCOL.s]<=X.s1
    && (E.players[r[SCOL.p]]||[])[5]===1 && (X.pi==null || r[SCOL.p]===X.pi)
  ...
  rows.forEach(r=>{
    ...
    active.forEach(m=>{ if(m.skillOnly&&['K','D/ST'].includes(EPOS[r[SCOL.p]])) return; const v = m.sNum ? m.sNum(r) : null; if(v!=null&&!isNaN(v)) g.sums[m.k]=(g.sums[m.k]||0)+v; });
  });
  let list=[...groups.values()];
  const sm = active.find(m=>m.k===X.sort)||active[0];
  const val=(g,m)=>g.sums[m.k]??null;
  if(X.md>0) list=list.filter(g=>g.n>=X.md);
  if(sm) list.sort((a,b)=>((val(b,sm)??-1e18)-(val(a,sm)??-1e18))*(X.dir<0?1:-1));
  return {list:list.slice(0,X.lim), scanned:rows.length, total:list.length, gd, active, val, ever:true};
}
...
function sentence(){
  ...
  const fr = X.sc!=='ever'&&X.f.length? ' by '+X.f.map(f=>'<b>'+esc((F[f]||{}).display_name)+'</b>').join(', ') : '';
  const player = X.pi!=null? `, player <b>${esc(ENAME[X.pi]||'')}</b>` : '';
  const pos = X.pos.length? ', '+X.pos.join('/') : '';
  const wk = X.sc!=='ever'&&(X.w0!==1||X.w1!==18)? `, weeks ${X.w0}–${X.w1}`:'';
  const md = X.md>0? `, minimum ${X.md} ${(MK[X.sort]&&MK[X.sort].den)?(MK[X.sort].minLabel||'sample'):(X.sc==='ever'?'games':'weeks')}`:'';
  return `${msL} by <b>${by}</b>, ${scope}${fr}${player}${pos}, <b>${X.s0}–${X.s1}</b>${wk}${md}, sorted by <b>${esc((MK[X.sort]||{}).l||'')}</b>.`;
}
...
function runQuery(){
  ...
  if(X.ms.some(k=>MK[k]&&MK[k].skillOnly) && (!X.pos.length||X.pos.some(p=>['K','D/ST'].includes(p)))) warns.push('kicking and D/ST are outside xFP/FPOE scope; those cells are unavailable');
  ...
  const head = `<tr><th class="l">#</th><th class="l">${esc((GRAINS.find(g=>g[0]===X.gr)||[])[1]||'')}</th><th>${agg.ever?'G':'Wks'}</th>${active.map(m=>`<th class="sortable ${m.k===X.sort?'sorted':''}" data-ms="${m.k}">${m.l}${m.den?'*':''}</th>`).join('')}</tr>`;
  const body = list.map((g,i)=>`<tr class="click xrowr" data-k="${esc(String(g.k))}">
    ...
    ${active.map(m=>{ const v=val(g,m); return `<td data-v="${v==null?-1e18:v}" class="${(['fpoe','fpoe2','benchAffl'].includes(m.k))?cls(v):''}">${m.fmt(v)}</td>`; }).join('')}</tr>
...
function exportCsv(){
  ...
  const lines = [
    ...
    ['group',lastAgg.ever?'games':'weeks',...active.map(m=>m.l)].map(s=>'"'+String(s).replace(/"/g,'""')+'"').join(',')
  ];
  list.forEach(g=>{
